...
def update_or_insert_data(file_name_video, file_name_audio, url_video, url_audio, transcript, id=None):
    try:
        with sqlite3.connect('audio_data.db') as conn:
            ...
            if id is not None:
                ...
                if exists:
                    ...
                    c.execute('''
                        UPDATE audio_data 
                        SET file_name_video=?, file_name_audio=?, url_video=?, url_audio=?, transcript=?
                        WHERE id=?
                    ''', (file_name_video, file_name_audio, url_video, url_audio, transcript, id))
                ...
                print("Inserted new record with auto-generated ID")
            conn.commit()
            ...
            if item['name'] == 'Audio':
                ...
                if not audio_files:
                    ...
                    for audio_file in audio_files:
                        ...
                        if output:
                            ...
                            update_or_insert_data('', audio_file['name'], '', url, output, id=current_id_audio)
                            current_id_audio += 1
                            ...
                            for filename in os.listdir(local_text_folder):
                                if filename.endswith('HV.txt'):
                                    ...
                                    if os.path.isfile(mentor_file):
                                        ...
                                        if output:
                                            update_or_insert_data('', '', '', '', output, id=current_id_audio)
                                            current_id_audio += 1
                ...
                if not video_files:
                    ...
                    for video_file in video_files:
                        ...
                        update_or_insert_data(video_file['name'], '', url, '', '', id=current_id_video)
                        current_id_video += 1
