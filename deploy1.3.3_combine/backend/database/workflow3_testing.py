...
class VideoProcessor:
    ...
    def debug_folder_access(self, folder_id: str):
        """
        Debug folder access and structure
        """
        try:
            self.logger.info(f"\n=== Starting Folder Debug for ID: {folder_id} ===")
            
            # 1. Check folder existence and permissions
            try:
                ...
                self.logger.info(f"Folder details:")
                self.logger.info(f"- Name: {folder.get('name')}")
                self.logger.info(f"- Type: {folder.get('mimeType')}")
                self.logger.info(f"- Owner: {folder.get('owners', [{'emailAddress': 'unknown'}])[0]['emailAddress']}")
                
            except Exception as e:
                self.logger.error(f"Cannot access folder: {str(e)}")
                return False
                
            # 2. List all items in folder
            ...
            # Group items by type for better visibility
            folders = []
            files = []
            ...
            for folder in folders:
                ...
                try:
                    ...
                    sub_query = f"'{folder['id']}' in parents"
                    sub_results = self.drive_service.files().list(
                        q=sub_query,
                        fields="files(id, name, mimeType)",
                        pageSize=10
                    ).execute()
                    sub_items = sub_results.get('files', [])
                    self.logger.info(f"  Contains {len(sub_items)} items")
                    for sub_item in sub_items[:5]:  # Show first 5 items only
                        self.logger.info(f"    > {sub_item['name']}")
                except Exception as e:
                    self.logger.error(f"  Cannot access subfolder contents: {str(e)}")
    ...
    def process_folder(self, folder_id: str):
        """
        Enhanced process_folder with debugging
        """
        try:
            self.logger.info("\n=== Starting Folder Processing ===")
            
            # Run debug check first
            if not self.debug_folder_access(folder_id):
                self.logger.error("Failed to access folder - stopping processing")
                return
            ...
            if not audio_folder or not video_folder:
                ...
                self.logger.error(f"Audio folder found: {bool(audio_folder)}")
                self.logger.error(f"Video folder found: {bool(video_folder)}")
                return
...
def main():
    ...
    try:
        ...
        # Test với folder không hoạt động
        problem_folder = "15ik6vWdz1IcbPmsPfr4p-DdszkB484yg"
        logger.info("\n=== Testing Problem Folder ===")
        processor.process_folder(problem_folder)
        
        # Test với folder hoạt động (thêm ID folder hoạt động vào đây)
        working_folder = "1_oVkhMaU4M1-ZIkb5sL4XSRE1LmZUn3z"  # Thay thế bằng ID folder đang hoạt động
        logger.info("\n=== Testing Working Folder ===")
        processor.process_folder(working_folder)
